...
def pool(x, ksize, stride, padding=u"SAME"):
...
class NIN(BaseModel):
  ...
  NINを用いた特徴量
  """
  default_caffemodel = u"nin_imagenet.caffemodel"
  default_alpha = [0., 0., 1., 1.]
  default_beta = [1., 1., 1., 1.]

  def __call__(self, x):
    u"""NINの特徴量"""
    x0 = self.conv(u"conv1")(x, stride=4)

    y1 = self.conv(u"cccp2")(self.conv(u"cccp1")(x0), activation_fn=None)
    pool1 = pool(tf.nn.relu(y1), ksize=3, stride=2)
    x1 = self.conv(u"conv2")(pool1, stride=1)

    y2 = self.conv(u"cccp4")(self.conv(u"cccp3")(x1), activation_fn=None)
    pool2 = pool(tf.nn.relu(y2), ksize=3, stride=2)
    x2 = self.conv(u"conv3")(pool2, stride=1)

    y3 = self.conv(u"cccp6")(self.conv(u"cccp5")(x2), activation_fn=None)
    pool3 = pool(tf.nn.relu(y3), ksize=3, stride=2)

    drop = tf.nn.dropout(pool3, 0.5)
    x3 = self.conv(u"conv4-1024")(drop)
...
class VGG(BaseModel):
  ...
  VGGを用いた特徴量
  """
  default_caffemodel = u"VGG_ILSVRC_16_layers.caffemodel"
  default_alpha = [0., 0., 1., 1.]
  default_beta = [1., 1., 1., 1.]
...
class Generator(object):
  ...
  def generate(self, config):
    with tf.Session() as sess:
      if hasattr(tf, u"global_variables_initializer"):
      ...
      for i in range(config.iteration):
        ...
        if (i + 1) % 50 == 0:
          ...
          print(u"%d| loss: %f, loss_orig: %f, loss_style: %f" % (i + 1, l, sum(l1), sum(l2)))
          for l1_, l2_ in zip(l1, l2):
            print(u"loss_orig: %f, loss_style: %f" % (l1_, l2_))
